...
def conv_bn(
    norm_type, in_channels, out_channels, kernel_size, stride, padding, groups=1
):
...
class RepVGG(nn.Module):
    def __init__(
        ...
        self.stage1 = self._make_stage(
            int(64 * width_multiplier[0]), norm_type, num_blocks[0], stride=2
        )
        self.stage2 = self._make_stage(
            int(128 * width_multiplier[1]), norm_type, num_blocks[1], stride=2
        )
        self.stage3 = self._make_stage(
            int(256 * width_multiplier[2]), norm_type, num_blocks[2], stride=2
        )
        self.stage4 = self._make_stage(
            int(512 * width_multiplier[3]), norm_type, num_blocks[3], stride=last_stride
        )
